...
6. **验证**每个任务：完成后用最窄的有效命令或检查验证。如果验证失败，在本地修复并重跑。绝不要声称验证已通过，除非它实际运行并通过。
7. 在已批准任务内持续推进，不需要额外批准，除非范围变化、验证结果改变计划，或需要用户输入。

所有任务完成后：
- 运行一次可用且有意义的最广范围验证（完整测试套件或集成检查）——不仅是逐任务检查。
- 审查完整 diff，检查集成缺口、契约不匹配和回归。在报告前解决问题。
- 总结修改内容、运行了哪些验证以及任何剩余问题。
